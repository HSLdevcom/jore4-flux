...
1. Test locally that everything works:
   1. with docker-compose: `docker-compose -f clusters/docker-compose.yaml up`
   1. with Kind: `./kindcluster start --cluster=clusters/e2e`
   1. with Azure: commit your changes, move `playg` branch to your the commit and wait for Flux to
      deploy it for you
1. Testing with CI/CD:
   1. Kind cluster remote setup: allow overwriting the docker image from an environment variable.
      See `/remotecluster.sh`
   1. Kind cluster github testing: do healtchchecks in all (remote and local) jobs in
      `.github/workflows/test-e2e-cluster-setup.yml`
   1. Docker compose github testing: do healtchchecks in
