...
To start Kind, execute `./kindcluster.sh kind:start`, to stop, run `./kindcluster.sh kind:stop`.
...
```
./kindcluster.sh setup:all --cluster=clusters/e2e
```
...
down. We deploy the resources directly.
